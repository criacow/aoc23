...
high_score = 0
for runnum in range(max_x):
  initial_beam = str(runnum) + ",-1,S"
  beams = [ initial_beam ]
  repeat_counter = 0
  last_energized = 0
  score = 0
  ...
  while repeat_counter < 125:
    ...
    if last_energized == len(energized):
      repeat_counter += 1
    else:
      last_energized = len(energized)
      repeat_counter = 0
    print("run: " + str(runnum) + ", hs: " + str(high_score) + ", beams: " + str(len(beams)) + ", energized: " + str(len(energized)) + ", repeat: " + str(repeat_counter))
    score = len(energized)
  if score > high_score:
    high_score = score
